...
def load_data(n_clients,dataset="MNIST", data_dir="./data", reduced=False, normalize="image-wise", flatten=False, device="cpu"):
    ...
    if dataset == "CIFAR10":
        ...
        train_input, train_target = DM.celebA_loader(corr,1200,n_clients)
        
        print("Load validation data...")
        test_input, test_target = DM.celebA_loader(corr,1200,n_clients)
...
class OutputTracker():
    ...
    def get_global_outputs(self,self_id, r=-1, n_avg=None, client_id=None):
        ...
        global_dis = torch.empty(self.meta["n_class"], self.n_clients)
